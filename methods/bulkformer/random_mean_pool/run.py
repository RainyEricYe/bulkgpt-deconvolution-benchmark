#!/usr/bin/env python3
"""BulkFormer random-weight + mean pooling + RidgeCV (control experiment).

Combines two control conditions: (a) randomly initialized BulkFormer
weights (no pretrained checkpoint) and (b) mean pooling over the full
encoder output instead of ``global_expr_proj``.

This is condition 4 in ``eval_check_pretraining.py`` ("Random + mean").

Usage::

    python methods/bulkformer/random_mean_pool/run.py \\
        --h5 data/2_real_bulk/sdy67.h5 \\
        --ground-truth data/2_real_bulk/sdy67_gt.csv \\
        --output-dir results/2_realbulk/sdy67/bulkformer/random_mean_pool
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="BulkFormer random-weight + mean pooling + RidgeCV",
    )
    ...
    with ResourceTracker() as rt:
        # ── Build random-weight encoder, mean-pool ───────────────────────
        t0 = time.monotonic()
        ...
        if args.loo:
            ...
            save_loo_results(out_dir, result, meta_extra={
                **rt.to_dict(backbone="bulkformer/random_mean_pool", dataset=dataset_name),
                "seed": args.seed,
                "loo": True,
            })
        else:
