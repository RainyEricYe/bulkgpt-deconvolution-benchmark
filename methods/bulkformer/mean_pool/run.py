#!/usr/bin/env python3
"""BulkFormer pretrained + full encoder + mean pooling + RidgeCV.

Runs the full BulkFormer-147M encoder (GCN + Performer) and averages
over the gene dimension to obtain sample-level embeddings, instead of
using the fast ``global_expr_proj`` MLP head.  The pretrained checkpoint
is loaded as usual.

Mean pool is condition 5 in ``eval_check_pretraining.py``
("Pretrained + mean").

Usage::

    python methods/bulkformer/mean_pool/run.py \\
        --h5 data/2_real_bulk/sdy67.h5 \\
        --ground-truth data/2_real_bulk/sdy67_gt.csv \\
        --output-dir results/2_realbulk/sdy67/bulkformer/mean_pool
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="BulkFormer pretrained + mean pooling + RidgeCV",
    )
    ...
    with ResourceTracker() as rt:
        ...
        emb = encode_bulkformer(vals, genes, samples, pooling="mean")
        rt.end_encode()
        ...
        if args.loo:
            ...
            save_loo_results(out_dir, result, meta_extra={
                **rt.to_dict(backbone="bulkformer/mean_pool", dataset=dataset_name),
                "seed": args.seed,
                "loo": True,
            })
        else:
