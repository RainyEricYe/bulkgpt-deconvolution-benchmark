...
library(MASS); X <- t(sig)
n_s <- nrow(bulk_t)
est <- matrix(0, nrow=n_s, ncol=n_types)
colnames(est) <- unique_types; rownames(est) <- rownames(bulk_t)
for (i in seq_len(n_s)) {
    y <- as.numeric(bulk_t[i, ]); w <- 1/(X^2+0.01); W <- diag(colSums(w))
    b <- tryCatch(pmax(ginv(t(X)%*%W%*%X) %*% t(X)%*%W%*%y, 0),
                  error=function(e) rep(1/n_types, n_types))
    s <- sum(b); if (s > 0) b <- b/s
    est[i, ] <- b
}
message(sprintf("Result: %d x %d", nrow(est), ncol(est)))
DeconUtils::writeH5(NULL, est, "SQUID-DWLS")
